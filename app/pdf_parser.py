...
class ParseResult:
    ...
    location: str = ""
    # Extensible: add source: str, shifts: Dict, warnings: List[str] here later
...
_LOCATION_RE = re.compile(r"Apotheke\s+(?:in|im)\s+([^\]]+)", re.IGNORECASE)

_HEADER_WORDS = frozenset(["wochenplan", "kw", "datum", "name", "tag", "kalenderwoche"])


def _extract_location(text: str) -> str:
    """Extract pharmacy location name from PDF header text.

    Matches patterns like:
      'Apotheke in Marschacht Offizin'  → 'Marschacht'
      'Apotheke im Garberscenter'       → 'Garberscenter'

    Callers can add an email-subject fallback without touching this function.
    """
    m = _LOCATION_RE.search(text)
    if not m:
        return ""
    loc = re.sub(r"\s+Offizin\s*$", "", m.group(1), flags=re.IGNORECASE).strip()
    return loc
...
class PDFParser:
    ...
    def parse(self, pdf_path: str) -> ParseResult:
        """Extract date→names mapping and location from a Wochenplan PDF.

        Returns an empty ParseResult on any error — never raises.
        Names within each day are sorted alphabetically.
        """
        if not _HAS_PDFPLUMBER:
            self.logger.error("pdfplumber not installed — cannot parse PDFs")
            return ParseResult(days={})
        ...
        if location:
            self.logger.info(f"Detected location: '{location}'")
        else:
            self.logger.warning(f"Could not detect location from PDF header in {pdf_path}")

        raw_days = self._run_strategies(text, pdf_path)
        sorted_days = {k: sorted(v) for k, v in raw_days.items()}

        if sorted_days:
            total = sum(len(v) for v in sorted_days.values())
            self.logger.info(f"Parsed {len(sorted_days)} day(s), {total} name(s) from {pdf_path}")
        else:
            self.logger.warning(
        ...
        return ParseResult(days=sorted_days, location=location)
